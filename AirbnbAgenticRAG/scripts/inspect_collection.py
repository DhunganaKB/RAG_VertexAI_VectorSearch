"""
Inspect Vector Search 2.0 Collection Records (Airbnb)
=====================================================
Displays the first N DataObjects stored in the collection, showing:
  - System fields  (ID, create/update time)
  - Listing data   (name, neighbourhood, price, room_type, rating, etc.)
  - Embedding      (dimensions + first 8 / last 4 values + stats)

Usage:
    python scripts/inspect_collection.py              # show first 3 records
    python scripts/inspect_collection.py --n 5        # show first 5 records
    python scripts/inspect_collection.py --n 1 --full # full text + all vector values
"""
...
# Fields to request from the collection
_DATA_FIELDS = [
...
def inspect(n: int = 3, full: bool = False) -> None:
    ...
    for i, obj in enumerate(objects, 1):
        ...
        print(f"{BOLD}{GREEN}┌─ Listing {i} {'─' * 60}{RESET}")
        ...
        # ── Listing metadata ──────────────────────────────────────────────────
        print(f"{BOLD}│{RESET}")
        print(f"{BOLD}│ {YELLOW}── LISTING ──{RESET}")
        print(f"{BOLD}│ name            {RESET}: {data.get('name', '—')}")
        print(f"{BOLD}│ neighbourhood   {RESET}: {data.get('neighbourhood', '—')}")
        print(f"{BOLD}│ property_type   {RESET}: {data.get('property_type', '—')}")
        print(f"{BOLD}│ room_type       {RESET}: {data.get('room_type', '—')}")
        print(f"{BOLD}│ accommodates    {RESET}: {data.get('accommodates', '—')}")
        print(f"{BOLD}│ bedrooms        {RESET}: {data.get('bedrooms', '—')}")
        print(f"{BOLD}│ price/night     {RESET}: ${data.get('price', 0):.2f}")
        print(f"{BOLD}│ rating          {RESET}: {data.get('rating', '—')}")
        print(f"{BOLD}│ instant_book    {RESET}: {data.get('instant_bookable', '—')}")
        print(f"{BOLD}│ host            {RESET}: {data.get('host_name', '—')}")
        if data.get("listing_url"):
            print(f"{BOLD}│ url             {RESET}: {data['listing_url']}")

        # ── Embedding text ────────────────────────────────────────────────────
        print(f"{BOLD}│{RESET}")
        print(f"{BOLD}│ {YELLOW}── EMBEDDING TEXT ──{RESET}")
        text_block = fmt_text(data.get("text", "—"), full=full)
        print(f"{BOLD}│ text            {RESET}: {text_block}")

        # ── Vector ───────────────────────────────────────────────────────────
        print(f"{BOLD}│{RESET}")
        print(f"{BOLD}│ {YELLOW}── EMBEDDING VECTOR ──{RESET}")
        if emb_values:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Inspect VS2.0 DataObjects for Airbnb Listings collection"
    )
    parser.add_argument("--n",    type=int, default=3,
                        help="Number of records to display (default: 3)")
    parser.add_argument("--full", action="store_true",
                        help="Show full text and all 768 vector values")
