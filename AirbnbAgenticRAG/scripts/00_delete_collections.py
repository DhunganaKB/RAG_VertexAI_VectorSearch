...
IMPORTANT: VS2.0 requires a collection to be empty before deletion.
This script automatically purges all DataObjects first.

Run:
    python scripts/00_delete_collections.py             # interactive
    python scripts/00_delete_collections.py --all       # delete all without prompting
    python scripts/00_delete_collections.py --id airbnb-listings-collection
"""
...
def purge_data_objects(collection_name: str) -> int:
    ...
    # Page through ALL DataObjects (no filter = match everything)
    while True:
...
def delete_indexes(client: vs.VectorSearchServiceClient, collection_name: str) -> int:
    """Delete all ScaNN ANN indexes attached to *collection_name*.

    VS2.0 raises 400 FailedPrecondition if you try to delete a collection
    that still has nested index resources.  Indexes must be removed first.
    Returns the number of indexes deleted.
    """
    try:
        indexes = list(client.list_indexes(
            request=vs.ListIndexesRequest(parent=collection_name)
        ))
    except Exception:
        return 0
    ...
    for idx in indexes:
        ...
        print(f"    Deleting index '{idx_id}'...", end=" ", flush=True)
        op = client.delete_index(request=vs.DeleteIndexRequest(name=idx.name))
        op.result()          # block until the LRO completes
        print("DONE")
...
def delete_one(client: vs.VectorSearchServiceClient, name: str) -> None:
    ...
    # Step 1 — remove all DataObjects (collection must be empty)
    print(f"  Purging DataObjects from '{col_id}'...", end=" ", flush=True)
    removed = purge_data_objects(name)
    print(f"{removed} removed")

    # Step 2 — delete nested ScaNN indexes (must happen before delete_collection)
    print(f"  Checking for nested indexes in '{col_id}'...")
    n_indexes = delete_indexes(client, name)
    if n_indexes == 0:
...
def main() -> None:
    ...
    if args.id:
        name = (f"projects/{config.PROJECT_ID}/locations/{config.LOCATION}"
                f"/collections/{args.id}")
